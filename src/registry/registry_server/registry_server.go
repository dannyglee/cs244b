...
// TODO(jin): merge this file with the main file.
import (
	"sync"
	"bytes"
	"encoding/json"
...
func (registry *Registry) ShowMembership(w http.ResponseWriter, r *http.Request) {
	w.Write([]byte(fmt.Sprintf("%v", registry.currentMembers)))
}
...
func Run(healthPingIntervalSeconds int, maxAllowablePingFailures int) {
	...
	mux.HandleFunc("/addSingleMember", registry.HandleAddSingleMember)
	mux.HandleFunc("/removeSingleMember", registry.HandleRemoveSingleMember)
	mux.HandleFunc("/updateGroup", registry.HandleMembershipChange)
	// ShowMembership
	mux.HandleFunc("/showMembers", registry.ShowMembership)
